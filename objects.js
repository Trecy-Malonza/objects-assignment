...
//    { name: 'Max', age: 19 },
   ...
   const confirmAge = people.filter(item =>{
    return(item.age>=18)
   ...
   //The function should return an object that groups the products by their categories, with the category names as keys and the arrays 
   //of products as values.
        ...
        [{ name: 'Laptop', price:1200, category: 'Electronics' },
         ...
         const groupedProducts =Object.groupBy(products,({category}) =>{
          return category;
         })
         ...
         { name: 'John', scores: [90, 80, 85] },
          ...
          function newArray(){
            return students.filter(student => {
              let avg =student.scores.reduce((acc, cur) => acc + cur) / student.scores.length;
              return avg >=85; 
            })
...
// display the car's information, write a function that takes the car object and adds a new method to the object called age. 
...
const car = {
 ...
 displayInfo: function() {
  ...
  age: function(){
    const currentCarAge = new Date ().getFullYear();
    const CarAge = currentCarAge - this.year
    return CarAge;
  }
}
console.log(car.age());
